...
export function sanitizeIpEntry(value: string): string | null {
  const trimmed = normalizeClientIp(value);
  if (!trimmed) return null;
  if (trimmed.includes("/")) {
    const [base, prefixRaw] = trimmed.split("/");
    const prefix = Number.parseInt(prefixRaw ?? "", 10);
    if (ipv4ToInt(base ?? "") === null || Number.isNaN(prefix) || prefix < 0 || prefix > 32) {
      return null;
    }
    return `${base}/${prefix}`;
  }
  return ipv4ToInt(trimmed) ? trimmed : null;
}